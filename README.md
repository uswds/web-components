...
This is an npm workspaces monorepo. The repository root is a private orchestrator (it is not itself published); the publishable packages live under `packages/`: `packages/elements` (published as `@uswds/elements`) and `packages/tokens` (published as `@uswds/tokens`). Each workspace package is versioned and released independently through the same Changesets flow described below — a changeset can target one or more packages, and only the packages it lists get a version bump.
